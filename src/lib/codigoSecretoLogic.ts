// Codigo Secreto - Core Game Logic
...
export function canStartGame(game: GameState): { canStart: boolean; reason?: string } {
  ...
  if (!redSpymaster) {
    return { canStart: false, reason: 'El equipo Rojo necesita un Jefe de Espias' };
  }
  if (!blueSpymaster) {
    return { canStart: false, reason: 'El equipo Azul necesita un Jefe de Espias' };
  }
...
export function giveClue(
  ...
  if (!player || player.role !== 'spymaster') {
    return { game, error: 'Solo el Jefe de Espias puede dar pistas' };
  }
...
export function proposeCard(
  ...
  if (!player || player.role !== 'spymaster') {
    return { game, error: 'Solo el Jefe de Espias puede proponer cartas' };
  }
