...
export function QRCode({ roomCode }: QRCodeProps) {
  ...
  const joinUrl = typeof window !== 'undefined'
    ? `${window.location.origin}/impostor/unirse/${roomCode}`
    : '';
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Unete a Impostor',
          text: `Unete a mi partida de Impostor! Codigo: ${roomCode}`,
          url: joinUrl,
        });
      } catch (error) {
  ...
  return (
    <div className="bg-slate-800/50 rounded-2xl p-4 border border-slate-700">
      ...
      <div className="text-center mb-4">
        ...
        <p className="text-3xl font-mono font-bold text-red-400 tracking-wider">
          {roomCode}
        </p>
      </div>
      ...
      <div className="flex gap-2">
        ...
        <button
          onClick={shareLink}
          className="flex-1 py-2 px-4 rounded-xl font-medium bg-red-600 text-white hover:bg-red-700 transition-colors flex items-center justify-center gap-2"
        >
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
