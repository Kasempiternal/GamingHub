// API Routes for Codigo Secreto game management
  ...
  try {
    ...
    switch (action) {
      ...
      case 'updatePlayer': {
        ...
        if (role === 'spymaster' && team) {
          ...
          if (existingSpymaster) {
            return jsonResponse({
              success: false,
              error: `El equipo ${team === 'red' ? 'Rojo' : 'Azul'} ya tiene Jefe de Espias`,
            }, 400);
          }
        }
      ...
      case 'start': {
        ...
        const player = game.players.find(p => p.id === playerId);
        if (!player?.isHost) {
          return jsonResponse({ success: false, error: 'Solo el anfitrion puede iniciar' }, 403);
        }
      ...
      case 'clue': {
        ...
        if (!word || word.trim().length < 1) {
          return jsonResponse({ success: false, error: 'Pista invalida' }, 400);
        }

        if (typeof number !== 'number' || number < 0 || number > 9) {
          return jsonResponse({ success: false, error: 'Numero invalido (0-9)' }, 400);
        }
      ...
      case 'guess': {
        ...
        if (typeof cardIndex !== 'number' || cardIndex < 0 || cardIndex > 24) {
          return jsonResponse({ success: false, error: 'Carta invalida' }, 400);
        }
      ...
      case 'reset': {
        ...
        const player = game.players.find(p => p.id === playerId);
        if (!player?.isHost) {
          return jsonResponse({ success: false, error: 'Solo el anfitrion puede reiniciar' }, 403);
        }
      ...
      case 'respondToProposal': {
        ...
        if (typeof accept !== 'boolean') {
          return jsonResponse({ success: false, error: 'Respuesta invalida' }, 400);
        }
      ...
      default:
        return jsonResponse({ success: false, error: 'Accion no valida' }, 400);
    }
  } catch (error) {
