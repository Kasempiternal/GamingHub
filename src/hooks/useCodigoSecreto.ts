...
interface UseCodigoSecretoOptions {
  roomCode: string;
  playerId: string | null;
  pollInterval?: number;
}

interface UseCodigoSecretoReturn {
  game: GameState | null;
  player: Player | null;
  loading: boolean;
  error: string | null;
...
export function useCodigoSecreto({ roomCode, playerId, pollInterval = 2000 }: UseCodigoSecretoOptions): UseCodigoSecretoReturn {
  ...
  const fetchGame = useCallback(async () => {
    ...
    try {
      const url = new URL('/api/codigo-secreto', window.location.origin);
